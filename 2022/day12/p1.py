...
def solve(pos,seen, grid,goal):
    @lru_cache
    def rec(pos):
        seen.append(pos)
        print("pos {} : label {}".format(pos, grid[pos[0]][pos[1]]))
        mini = float("inf")
        mini_i = None
        if pos == goal:
            return 0
        for v in get_voisins(*pos, grid):
            # print(get_voisins(*pos, grid))
            if v in seen:
                continue
            print(ord(grid[v[0]][v[1]]) - ord(grid[pos[0]][pos[1]]))
            if (ord(grid[v[0]][v[1]]) - ord(grid[pos[0]][pos[1]])) <= 1:
                res = rec(v)
                if mini > res:
                    mini = res
                    mini_i = v
        print("mini was {}".format(mini_i))
        print("depile {} : label {}".format(pos, grid[pos[0]][pos[1]]))
        seen.pop()
        return mini + 1
    return rec(pos)
