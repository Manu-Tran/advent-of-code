...
def simulate(robots, state, blueprint):
    ...
    def rec(robots, state, timer, id):
        ...
        if timer == 1:
            ...
            return robots, state, str(25-timer) + "pass-"
        ...
        if not(flag) and sum(new_state) < 50:
        # if not(flag):
            ...
            if s[3] > max_s[3]:
                ...
                cur_c = "pass-"
        # print(max_s, timer)
        return max_r, max_s, max_c + str(25-timer) + cur_c
    print(blueprint["id"])
    return rec(robots, state, 24, blueprint["id"])
...
def run():
    ...
    N = 24
    scores = list()
    for b in reversed(blpts):
        print(b)
        scores.append((*simulate((1,0,0,0), (0,0,0,0), b), b["id"]))
        print(scores)
    res = []
    for score in scores:
        res.append((score[1][-1], score[3]))
    print(res)
    fscore = 0
    for r in res:
        fscore += r[0]*r[1]
